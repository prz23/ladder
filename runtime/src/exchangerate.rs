...
pub trait Trait: balances::Trait + session::Trait + bank::Trait {
...
decl_storage! {
    trait Store for Module<T: Trait> as Exchange {
        ...
        IdSignTxListC  get(all_list_c) : map u64 => Vec<T::AccountId>;
...
decl_module! {
    ...
    pub  fn check_exchange(origin, message: Vec<u8>, signature: Vec<u8>) -> Result{
        ...
        let (who , transcation  ,sign ) = Self::parse_data(message, signature);
...
impl<T: Trait> Module<T> {
    ...
    /// 签名并判断如果当前签名数量足够就发送一个事件
       /// Sign and determine if the current number of signatures is sufficient to send an event
    pub  fn check_signature(who: T::AccountId, transcation : u64 ,sign : u64) -> Result{
    ...
    ///    bit          32   32   32     8          8    8
    ///    实现         ok   no   ok     ok         ok   no
    fn parse_data(message: Vec<u8>, signature: Vec<u8>) -> (T::AccountId, u64 , u64){
        ...
        return (who, rate, time);
    }
    //let (who: T::AccountId, transcation : u64 ,sign : u64) = Self::parse_data();
}